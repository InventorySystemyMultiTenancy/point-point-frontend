    ...
    <div className="flex flex-col items-center justify-center min-h-screen animated-gradient p-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-10">
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-gray-800 mb-2">Point&Point</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 border-2 border-stone-200 rounded-lg focus:outline-none focus:border-purple-700 transition-colors"
              autoFocus
              disabled={isLoading}
            />
            {error && <p className="text-purple-700 text-sm mt-2">{error}</p>}
          </div>
          <button type="submit" disabled={isLoading} className="w-full bg-purple-700 text-white font-bold py-3 rounded-lg hover:bg-purple-800 transition-colors text-lg disabled:bg-purple-300">
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={checkDocument} className="space-y-6">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border-2 border-stone-200 rounded-lg focus:outline-none focus:border-purple-700 transition-colors text-lg"
              />
              {error && <p className="text-purple-700 text-sm mt-2">{error}</p>}
            </div>
            <button type="submit" disabled={isLoading} className="w-full bg-purple-700 text-white font-bold py-3 rounded-lg">
        ...
        {showPassword && (
          <form onSubmit={handleLoginWithPassword} className="space-y-6">
            ...
            <button type="submit" className="w-full bg-purple-700 text-white font-bold py-3 rounded-lg">Entrar</button>
        ...
        {requiresRegistration && (
          <form onSubmit={registerUser} className="space-y-4 max-h-[60vh] overflow-y-auto px-1">
            ...
            {error && <p className="text-purple-700 text-sm">{error}</p>}
            <button type="submit" className="w-full bg-purple-700 text-white font-bold py-3 rounded-lg">Cadastrar</button>
